      ...
      {this.state.showSuccess ? (<React.Fragment></React.Fragment>) : (null)}
        <List>
          {this.props.myActivity.map((item) => (
            <ActivityCard key={item.username} activityInfo={item.activityInfo} activityType={item.activityType} image={item.image} userName={item.username} userId={this.props.userId} activitySize="medium"/>
          ))}
        </List>
      </Box>
    );
